...
# the Mac app signs with. Run it on the Mac that holds the
# signing key:
#
...
# identity" in .github/workflows/desktop.yml.
#
...
KEEP_DIR="${KEEP_DIR:-$HOME/aethrakairos-signing}"
WORK="$(mktemp -d)"
...
if [ -n "$MISSING" ] || [ "$SIGNING_ON" != "true" ]; then
  ...
  printf '\nNext: a validation build, which signs and notarizes without publishing.\n  gh workflow run desktop.yml --repo %s --ref main -f channel=none\n' "$REPO"
fi
